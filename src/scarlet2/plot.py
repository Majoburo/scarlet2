...
def mcmc_scene(obs, scene, samples, vmin=None, vmax=None, figsize=(12, 4)):
    """Show mean(data), mean(model at posterior means), and residual."""
    s = copy.deepcopy(scene)
    for i, src in enumerate(s.sources):
        kc, ks, km, ksz, kel = f"center:{i}", f"spectrum:{i}", f"morph:{i}", f"size:{i}", f"ellipticity:{i}"
        if kc in samples: src.center = np.asarray(samples[kc]).mean(axis=0)
        if ks in samples: src.spectrum = np.asarray(samples[ks]).mean(axis=0)
        if km in samples: src.morphology = np.asarray(samples[km]).mean(axis=0)
        if ksz in samples and hasattr(src.morphology, "size"):
            src.morphology.size = float(np.asarray(samples[ksz]).mean())
        if kel in samples and hasattr(src.morphology, "ellipticity"):
            src.morphology.ellipticity = np.asarray(samples[kel]).mean(axis=0)

    model   = obs.render(s())
    n = jnp.prod(jnp.asarray(obs.data.shape)) - jnp.sum(obs.weights == 0)
    data2d  = np.asarray(obs.data).mean(axis=0)
    model2d = np.asarray(model).mean(axis=0)
    resid = (obs.weights * (obs.data - model) ** 2).sum(axis=0) / n


    fig, ax = plt.subplots(1, 3, figsize=figsize, constrained_layout=True)
    im0 = ax[0].imshow(data2d,  origin="lower", vmin=vmin, vmax=vmax); ax[0].set_title("Data")
    im1 = ax[1].imshow(model2d, origin="lower", vmin=vmin, vmax=vmax); ax[1].set_title("Model (post mean)")
    v = float(np.percentile(np.abs(resid), 98)) if np.isfinite(resid).any() else 1.0
    im2 = ax[2].imshow(resid,   origin="lower", cmap="RdBu_r", vmin=-v, vmax=v); ax[2].set_title("Residual")
    for im, a in [(im0, ax[0]), (im1, ax[1]), (im2, ax[2])]:
        fig.colorbar(im, ax=a, fraction=0.046)

    for r in range(1,3):
        ax[r].sharex(ax[0]); ax[r].sharey(ax[0])

    return fig, ax

def mcmc_diagnostics(obs, scene, samples, spectra=None, centers=None):
    """Minimal per-source diagnostics: mean image proxy, spectrum, center trace."""
    nsrc = len(scene.sources)
    fig, axes = plt.subplots(nsrc, 3, figsize=(12, 3 * max(1, nsrc)), constrained_layout=True)
    if nsrc == 1:
        axes = axes[np.newaxis, :]

    for i, _ in enumerate(scene.sources):
        breakpoint()
        # render only source i (others muted) at posterior means; average over channels
        s = copy.deepcopy(scene)
        for j, sj in enumerate(s.sources):
            if j != i:
                try: sj.spectrum = np.zeros_like(np.asarray(sj.spectrum))
                except Exception: pass
        kc, ks, km, ksz, kel = f"center:{i}", f"spectrum:{i}", f"morph:{i}", f"size:{i}", f"ellipticity:{i}"
        if kc in samples: s.sources[i].center = np.asarray(samples[kc]).mean(axis=0)
        if ks in samples: s.sources[i].spectrum = np.asarray(samples[ks]).mean(axis=0)
        if km in samples: s.sources[i].morphology = np.asarray(samples[km]).mean(axis=0)
        if ksz in samples and hasattr(s.sources[i].morphology, "size"):
            s.sources[i].morphology.size = float(np.asarray(samples[ksz]).mean())
        if kel in samples and hasattr(s.sources[i].morphology, "ellipticity"):
            s.sources[i].morphology.ellipticity = np.asarray(samples[kel]).mean(axis=0)

        img = np.asarray(obs.render(s())).mean(axis=0)
        axes[i, 0].imshow(img, origin="lower"); axes[i, 0].set_title(f"Source {i} mean")

        if f"spectrum:{i}" in samples:
            S = np.asarray(samples[f"spectrum:{i}"])
            mu, sig = S.mean(axis=0), S.std(axis=0)
            x = np.arange(mu.size)
            if spectra is not None:
                axes[i, 1].plot(spectra[i],color='black')
            axes[i, 1].plot(x, mu, "-o", ms=3)
            axes[i, 1].fill_between(x, mu - sig, mu + sig, alpha=0.25, linewidth=0)
        axes[i, 1].set_title(f"Spectrum {i}")

        if f"center:{i}" in samples:
            C = np.asarray(samples[f"center:{i}"])
            if centers is not None:
                C -= np.asarray(centers[i])
            axes[i, 2].plot(C[:, 0], ".", ms=2, label="y")
            axes[i, 2].plot(C[:, 1], ".", ms=2, label="x")
            if i == 0: axes[i, 2].legend(frameon=False, fontsize=9)
        axes[i, 2].set_title(f"Center {i}")
    return fig, axes

def corner_centers_spectra(samples, scene, sources=None, channels="all", thin=1, max_samples=5000):
    """Basic corner of centers and spectra."""
    nsrc = len(scene.sources)
    if sources is None: sources = list(range(nsrc))
    if channels == "all":
        C = scene.frame.C; chan_idx = list(range(C))
    else:
        chan_idx = list(channels)

    cols, labels = [], []
    for i in sources:
        kc, ks = f"center:{i}", f"spectrum:{i}"
        if kc in samples:
            Ci = np.asarray(samples[kc])[::thin]
            cols += [Ci[:, 0], Ci[:, 1]]; labels += [f"S{i} dy", f"S{i} dx"]
        if ks in samples:
            Si = np.asarray(samples[ks])[::thin]
            for k in chan_idx:
                if k < Si.shape[1]:
                    cols.append(Si[:, k]); labels.append(f"S{i} spec[{k}]")
    if not cols: raise ValueError("No center:/spectrum: samples")
    L = min(len(c) for c in cols)
    X = np.vstack([c[:L] for c in cols]).T
    if len(X) > max_samples:
        idx = np.random.choice(len(X), size=max_samples, replace=False); X = X[idx]
    fig = corner.corner(X, labels=labels, show_titles=True)
    return fig